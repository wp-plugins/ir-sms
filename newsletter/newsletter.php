			...
			if(!$check_mobile || $type != 'subscribe') {
				if($type == 'subscribe') {
					...
					if(get_option('wp_subscribes_activation')) {
						$key = rand(1000, 9999);
						$check = $wpdb->query("INSERT INTO {$table_prefix}sms_subscribes (date, name, mobile, status, activate_key) VALUES ('".$get_current_date."', '".$name."', '".$mobile."', '0', '".$key."')");
						...
						$check = $wpdb->query("INSERT INTO {$table_prefix}sms_subscribes (date, name, mobile, status) VALUES ('".$get_current_date."', '".$name."', '".$mobile."', '1')");
