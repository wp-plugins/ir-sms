...
<div class="wrap">
	...
	<form action="" method="post">
		<table class="widefat fixed" cellspacing="0">
			...
			<tbody>
				...
				$get_result = $wpdb->get_results("SELECT * FROM {$table_prefix}sms_subscribes");
				...
				<tr class="<?php echo $i % 2 == 0 ? 'alternate':'author-self'; ?>" valign="middle" id="link-2">
					...
					<td class="column-name"><img src="<?php echo bloginfo('url') . '/wp-content/plugins/'.WP_SMS_DIRNAME.'/images/' . $gets->status; ?>.png" align="middle"/></td>
					<td class="column-name"><a href="?page=<?=WP_SMS_DIRNAME;?>/subscribe&action=edit&ID=<?php echo $gets->ID; ?>"><?php _e('Edit', 'wp-sms'); ?></a></td>
				</tr>
				<?php
					}
				} else { ?>
	...
	<form action="" method="post">
		<table>
			<tr><td colspan="2"><h3><img src="<?php echo plugins_url(WP_SMS_DIRNAME.'/images/add.png');?>"/>  <?php _e('Add new subscribe:', 'wp-sms'); ?></h4></td></tr>
			<tr>
				<td><span class="label_td" for="wp_subscribe_name"><?php _e('Name', 'wp-sms'); ?>:</span></td>
